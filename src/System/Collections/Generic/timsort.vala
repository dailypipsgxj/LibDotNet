 ...
 */
internal class System.Collections.Generic.TimSort<G> : GLib.Object {

	public static void sort<G> (ICollection<G> list, GLib.CompareDataFunc<G> compare) {
		if (list is List) {
			TimSort.sort_list<G> ((List<G>) list, compare);
		} else {
			TimSort.sort_ICollection<G> (list, compare);
		}
	}

	private static void sort_ICollection<G> (ICollection<G> list, GLib.CompareDataFunc<G> compare) {
		TimSort<G> helper = new TimSort<G> ();

		helper.list_collection = list;
		//helper.array = list.to_array ();
		list.CopyTo(helper.array);
	...
	private static void sort_list<G> (List<G> list, GLib.CompareDataFunc<G> compare) {
		TimSort<G> helper = new TimSort<G> ();
	...
	private ICollection<G> list_collection;
	private G[] array;
	private void** list;
	private int index;
	private int size;
	...
	private unowned GLib.CompareDataFunc<G> compare;
	...
	private void do_sort () {
		...
		GLib.assert (remaining.index == size);

		merge_force_collapse ();

		GLib.assert (pending.length == 1);
		GLib.assert (pending[0].index == 0);
		GLib.assert (pending[0].length == size);
	}
	...
	private void insertion_sort (Slice<G> a, int offset) {
		...
		for (int start = a.index + offset; start < a.index + a.length; start++) {
			...
			GLib.assert (left == right);

			GLib.Memory.move (&a.list[left + 1], &a.list[left], sizeof (G) * (start - left));
			a.list[left] = pivot;
		}
	}
	...
	private void merge_at (int index) {
		...
		GLib.assert (a.length > 0);
		GLib.assert (b.length > 0);
		GLib.assert (a.index + a.length == b.index);
	...
	private int gallop_leftmost (G key, Slice<G> a, int hint) {
		...
		GLib.assert (-1 <= last_offset);
		GLib.assert (last_offset < offset);
		GLib.assert (offset <= a.length);
	...
	private class Slice<G> {
		...
		public void copy () {
			new_list = GLib.Memory.dup (&list[index], (uint) sizeof (G) * length);
			list = new_list;
			index = 0;
		}

		public inline void merge_in (void** dest_array, int index, int dest_index, int count) {
			GLib.Memory.move (&dest_array[dest_index], &list[index], sizeof (G) * count);
		}
